...
class LatentDiffusion(pl.LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        '''
        h_dims : network channels
        in_res : input image resolution
        attn_res : attention resolution
        down_index : resolution down index
        with_conv : indicating whether convolution is used or not for upblock
        dropout : dropout rate for resnet block
        latent_dim : I guess it is useless
        diffusion_steps :number of diffusion steps
        sampling_period : data generating interval
        grid : number of images sampled at once
        encoder : autoencoder's encoder
        decoder : autoencoder's decoder
        '''
        self.config = config

        # ============================================================
        self.h_dims = self.config.get('h_dims')
        self.in_res = self.config.get('in_res')
        self.attn_res = self.config.get('attn_res')
        self.down_index = self.config.get('down_index')
        self.with_conv = self.config.get('with_conv')
        ...
        self.encoder = self.config.get('encoder')
        self.decoder = self.config.get('decoder')
        # ============================================================
        ...
        f_res = self.in_res # [224, 112, 56, 28, 14, 7]
